...
编辑 `workers/wrangler.toml`，替换 `database_id`：
...
### Q: `[vars]` 中的 JWT_SECRET 能用 `wrangler secret put` 更新吗？
A: 不能。`[vars]` 中定义的变量需要用 `wrangler secret put` 会冲突。直接在 `wrangler.toml` 的 `[vars]` 中修改，或使用 `wrangler secret put JWT_SECRET` 前先从 `[vars]` 中删除该变量。
