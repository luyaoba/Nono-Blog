...
- **后台管理**：独立 Admin 面板（`/admin.html`），文章 CRUD、标签/分类持久化管理、点赞统计
- **站点配置**：头像、昵称、标语、背景图、页面标题等全部后台可配
- **图片上传**：R2 对象存储，支持头像、背景图、文章封面/正文图片上传，按分类 + 文章 ID 分子目录存储
- **图片缓存**：浏览器 30 天 + stale-while-revalidate，CDN 7 天，兼顾性能与删除后及时失效
...
- **速率限制**：登录（5 次/15 分钟）、点赞（30 次/小时）、上传（60 次/小时）
...
- **上传白名单**：仅允许 JPG/PNG/WebP/GIF/SVG，5MB 大小限制，按分类 + 文章 ID 分子目录
- **配置白名单**：站点配置接口仅允许预定义键名
- **状态白名单**：文章状态（published/draft）在后端枚举校验
- **输入长度限制**：标签、分类、文章等字段均在后端限制最大长度，防止异常数据
...
后端 API（Cloudflare Workers）
  ↓
D1 SQLite（文章/标签/分类/点赞/配置/浏览统计）  +  R2（图片存储）
```
...
│   ├── src/index.ts              # API 路由（文章/标签/分类/配置/上传/统计）
│   ├── migrations/               # 数据库迁移脚本
...
| POST | `/api/admin/upload` | 上传图片到 R2（≤5MB，按分类 + 文章 ID 分子目录） |
| GET | `/api/admin/daily-views` | 浏览统计（支持 week/month/year/all/custom 范围） |
| POST | `/api/admin/tags` | 创建标签 |
| PUT | `/api/admin/tags/:id` | 更新标签 |
| DELETE | `/api/admin/tags/:id` | 删除标签（有关联文章时禁止删除） |
| POST | `/api/admin/categories` | 创建分类 |
| PUT | `/api/admin/categories/:id` | 更新分类 |
| DELETE | `/api/admin/categories/:id` | 删除分类（有关联文章时禁止删除） |
...
### Q: 后台删除标签/分类后刷新又出现？
A: 确认 Worker 已部署最新版本，并且 AdminTags 组件已通过 `authToken` 调用 `/api/admin/tags` 或 `/api/admin/categories` 接口。删除操作会校验是否有关联文章。

### Q: 写文章时分类下拉不显示实际分类？
A: 已修复为读取 `categories.title` 而非 `colorName`，确保分类下拉与数据库实际分类一致。

### Q: 直接在 R2 删除图片后文章还能显示？
A: 原 `immutable` 1 年缓存策略导致浏览器/CDN 长期缓存。已调整为浏览器 30 天 + stale-while-revalidate、CDN 7 天，删除后可在更短时间内失效。
